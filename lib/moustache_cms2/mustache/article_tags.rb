    ...
    module ArticleTags
      ...
      def find_articles(name)
        if params[:tag]
          ...
          @articles = @current_site.articles_by_collection_name_and_tag(name.to_s, params[:tag]).desc('current_state.time').page(params[:page]).per(MoustacheCms::Application.config.default_per_page)
        else
